...
], function(declare, Core, dojoAttr, dojoClass, dojoStyle, dojoLang, dojoQuery, widgetTemplate) {
   "use strict";
   ...
   return declare("GridSearch.widget.GridSearch", [Core], {
      ...
      startup: function() {
         if (this.renderAsTextarea) {
            ...
            if (this.ta_rows > 0) {
              console.log("Setting textarea rows");
      ...
      _finishGridSetup: function() {
         if (this._grid) {
            ...
            if (
               this._grid.config &&
               this._grid.config.gridpresentation &&
               this._grid.config.gridpresentation.waitforsearch &&
            ...
            if (!this.expertQuery) {
               ...
               for (var i = 0, attr; (attr = attributes[i]); ++i) {
                  ...
                  if (!attr.customSearchEntity) {
                     //basic search
                     var searchPath = attr.searchAttributeParam.substring(
                        0,
                        attr.searchAttributeParam.lastIndexOf("/")
                     );
                     var searchAttribute = attr.searchAttributeParam.substring(
                        attr.searchAttributeParam.lastIndexOf("/") + 1,
                        attr.searchAttributeParam.length
                     );
                     ...
                     //advanced search
                     searchParams.push(
                        attr.customSearchPath +
                           "[" +
                           attr._searchMethod +
                           "(" +
                           attr.customSearchAttribute +
                           ",'" +
                           curValue +
                           "')]"
                     );
